  ...
  return (
    <section className="bg-[#FDF8F2] border-b border-[#E1D2BF]">
      <div className="mx-auto max-w-7xl px-6 py-16 sm:px-10 lg:py-20 xl:max-w-[1820px] xl:px-16 xl:py-24">
        ...
        <div className="mt-12 flex flex-col items-center gap-10 lg:mt-14 lg:flex-row lg:justify-center lg:gap-8 xl:gap-12">
          {howItWorksSteps.map((step, index) => (
            <div key={step.step} className="contents">
              <HowItWorksStep step={step} />

              {index < howItWorksSteps.length - 1 ? (
                <StepArrow className="hidden h-3 w-24 shrink-0 text-primary lg:block xl:w-28" />
              ) : null}
            </div>
          ))}
        </div>
